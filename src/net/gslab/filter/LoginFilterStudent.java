...
/**
 * Servlet Filter implementation class LoginFilterStudent
 */
@WebFilter("/LoginFilterStudent")
public class LoginFilterStudent implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		// TODO Auto-generated method stub
		System.out.println("进入学生登录验证过滤器：");
        ...
        if (request != null && request.getAttribute(FILTERED_REQUEST)!=null) {  //?
			...
			System.out.println("直接通过学生过滤器");
			chain.doFilter(request, response);
		} else {
			System.out.println("首次进入学生过滤器");
			String type=(String) httpRequest.getSession().getAttribute(CommonConstant.LOGIN_TYPE);
			if(  type==null|| type.equals("")||!type.equals("student")){   // 如果用户为空且未登录,或者不是学生
				// 拿到url
				...
				// 把想要访问的界面存到LOGIN_TO_URL里，转到登陆界面，一旦登陆成功就去那个页面
				request.setAttribute("ERROR_MSG_KEY", "您访问的页面需要有学生登陆权限，请先登陆！");
				httpRequest.getSession().setAttribute(
